...
contract("ZionodesTokenFactory", (accounts) => {
  ...
  it("is superadmin", async () => {
    assert.equal(
      true,
      await contract.isSuperAdmin(superadmin, { from: superadmin })
    );
  });

  it("add admin", async () => {
    await utils.shouldThrow(
      contract.addAdmin(bob, { from: alice }),
  ...
  it("remove admin", async () => {
    ...
    await utils.shouldThrow(
      contract.removeAdmin(alice, { from: bob }),
      "Restricted to super admins."
    );
  ...
  it("add and renounce superadmin", async () => {
    await utils.shouldThrow(
      contract.addSuperAdmin(bob, { from: alice }),
      "Restricted to super admins."
    );
    ...
    await utils.shouldThrow(
      contract.renounceSuperAdmin({ from: alice }),
      "Restricted to super admins."
    );
